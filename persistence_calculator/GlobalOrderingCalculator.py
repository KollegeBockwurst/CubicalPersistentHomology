...
def sort_position(search_list, iterator):
    return [search_list.index(i) for i in iterator]
...
class GlobalOrderingCalculator:
    ...
    def run(self):
        ...
        args = []
        chunk_size = (len(global_filtration) // self.thread_number) + 1
        for i in range(0, len(global_filtration), chunk_size):
            chunk = range(i, min(i + chunk_size, len(global_filtration)))
            args.append([sorted_indices, chunk])
        with Pool(len(args)) as p:
            p_results = p.starmap(sort_position, args)
        sorted_positions = []
        for p_result in p_results:
            sorted_positions.extend(p_result)
