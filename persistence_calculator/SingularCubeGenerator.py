...
class SingularCubeGenerator(threading.Thread):
    def __init__(self, graph: Graph, max_dim: int, start_vertex: int):
        threading.Thread.__init__(self)
        self.max_dim = max_dim
        self.graph = graph
        self.result = [[] for _ in range(max_dim + 1)]
        self.order_graph = graph.order()  # number of vertices in the given graph
        # sage adjacency is with zeros on main diagonal:
        self.adjacency_graph = graph.adjacency_matrix() + identity_matrix(self.order_graph)  # adjacency matrix of
        # the given graph, deep copy

        self.order_max_cube = pow(2, max_dim)  # number of vertices in a cube graph of dimension max_dim
        ...
        self.options = [[] for _ in range(self.order_max_cube)]  # create list of independent lists
        self.options[0] = [start_vertex]  # initialize options[0], since this is always possible
        self.cube_mapping = [None] * self.order_max_cube  # saves the current state of the mapping
    ...
    def run(self):
        ...
        while loop_flag:  # main loop
            ...
            for change_index in range(self.order_max_cube - 1, -1, -1):  # loop cube vertices from right to left
                if len(self.options[change_index]) > 0:  # check for existing options
                    self.cube_mapping[change_index] = self.options[change_index].pop(0)  # apply the option to
                    # cube_mapping
                    ...
                    if (change_index + 1) & change_index == 0:
                        ...
                        singular_cube = self.cube_mapping[0:change_index + 1]
                        ...
                        if not degeneracy_flag:
                            self.result[cube_dim].append(singular_cube)

                    # ----------
                    # generate the options for the next vertex (change_index + 1), if there is a next vertex:
                    if change_index < self.order_max_cube - 1:
                        next_options = []  # saves the found options for vertex change_index+1
                        for possible_option in range(
                                self.order_graph):  # loop through all possible options for the mapping
                            ...
                            for j in range(
                                    self.max_dim):  # loop through all adjacant vertices of (cube) vertex change_index+1
                                ...
                                if not self.adjacency_graph[possible_option][self.cube_mapping[vertex3]]:
                                    possible_flag = False
                                    break
                        ...
                        self.options[
                            change_index + 1] = next_options  # add all possible options to the outer options list
...
def generate_singular_cubes(adjacency_graph, max_dim: int, start_options):
    """
